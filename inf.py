...
def detect_image(weights,image_url,img_size,conf_thres,iou_thres):
    ...
    if len(result_boxes) > 0:
        ...
        for i,r in enumerate(result_boxes):
            ...
            org = (int(r[0]),int(r[1]))
            cv2.rectangle(img, (int(r[1]),int(r[3])), (int(r[2]),int(r[3])), (255,0,0), 1)
            cv2.putText(img, str(int(100*result_scores[i])) + '%  ' + str(result_class_names[i]), org, font,  
                        fontScale, color, thickness, cv2.LINE_AA)
            roi = img [int(r[1]):int(r[3]),int(r[0]):int(r[2])]
            save_result_filepath = '/home/pi/yolov5-export-to-raspberry-pi/matricule.jpg'
            cv2.imwrite(save_result_filepath,roi)
